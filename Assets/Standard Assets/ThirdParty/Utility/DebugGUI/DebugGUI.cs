...
public class DebugGUI : MonoBehaviour
{
    ...
    private static DebugGUI Instance
    {
        get
        {
            if (_instance == null)
            {
                ...
                if (_instance == null && Application.isPlaying) _instance = new GameObject("DebugGUI").AddComponent<DebugGUI>();
            }
    ...
    [SerializeField] private bool drawInBuild;

    [SerializeField] private bool displayGraphs = true;
    ...
    private readonly Dictionary<object, string> persistentLogs = new Dictionary<object, string>();
    private readonly Queue<TransientLog> transientLogs = new Queue<TransientLog>();
    private readonly Dictionary<object, GraphContainer> graphDictionary = new Dictionary<object, GraphContainer>();
    ...
    private static bool LogsEnabled => Instance.displayLogs && (Instance.drawInBuild || Application.isEditor);
    private static bool GraphsEnabled => Instance.displayGraphs && (Instance.drawInBuild || Application.isEditor);
    ...
    /// <summary>
    ///     Add a event data point to a graph.
    /// </summary>
    /// <param name="key">The graph's key</param>
    /// <param name="val">Value to be added</param>
    public static void GraphEvent(object key, float val)
    {
        if (GraphsEnabled)
            Instance.InstanceGraphEvent(key, val);
    }
    ...
    private void InstanceRemoveGraph(object key)
    {
        ...
        {
            GraphContainer graph = graphDictionary[key];
    ...
    {
        if (!graphDictionary.ContainsKey(key)) InstanceCreateGraph(key);
    ...
    private void InstanceGraphEvent(object key, float val)
    {
        if (!graphDictionary.ContainsKey(key)) InstanceCreateGraph(key);

        if (freezeGraphs) return;

        graphDictionary[key].PushEvent(val);
    }
    ...
    private void Update()
    {
        ...
        if (LogsEnabled || GraphsEnabled) CleanUpDeletedAtributes();

        if (LogsEnabled)
        {
            // Clean up expired logs
            while (transientLogs.Count > 0 && transientLogs.Peek().expiryTime <= Time.realtimeSinceStartup) transientLogs.Dequeue();
        }
        ...
        if (GraphsEnabled)
        {
            ...
            for (int i = 0; i < attributeContainers.Count; i++)
            {
                ...
                if (mb != null && attributeKeys.ContainsKey(mb))
                {
                    ...
                    foreach (AttributeKey key in attributeKeys[mb])
    ...
    private readonly GUIContent labelGuiContent = new GUIContent();
    ...
    private void DrawLogs()
    {
        ...
        for (int i = 0; i < attributeContainers.Count; i++)
        {
            ...
            if (mb != null)
            {
                ...
                {
                    foreach (FieldInfo field in debugGUIPrintFields[type])
                        DrawLabel(string.Format("{0} {1}: {2}", mb.name, field.Name, field.GetValue(mb)));
                }

                if (debugGUIPrintProperties.ContainsKey(type))
                {
                    foreach (PropertyInfo property in debugGUIPrintProperties[type])
                        DrawLabel(string.Format("{0} {1}: {2}", mb.name, property.Name, property.GetValue(mb, null)));
                }
            }
        }

        foreach (string log in persistentLogs.Values) DrawLabel(log);

        if (textRect.y != 0 && transientLogs.Count != 0) DrawLabel("-------------------");

        foreach (TransientLog log in transientLogs) DrawLabel(log.text);
    ...
    private readonly HashSet<int> graphGroupBoxesDrawn = new HashSet<int>();
        ...
        foreach (int group in graphGroupBoxesDrawn)
        {
            ...
            foreach (GraphContainer graph in graphDictionary.Values)
            {
                ...
                if (graph.group == group)
                {
                    ...
                    GUI.Label(new Rect(Screen.width - graphWidth - minMaxXOffset, groupOrigin, minMaxXOffset, graphHeight), labelGuiContent,
                        minMaxTextStyle);
                    // Min
                    labelGuiContent.text = graph.min.ToString("F2");
                    GUI.Label(
                        new Rect(Screen.width - graphWidth - minMaxXOffset, groupOrigin + graphHeight - textSize.y, minMaxXOffset,
                            graphHeight), labelGuiContent, minMaxTextStyle);
    ...
    public static void ForceReinitializeAttributes()
    {
        ...
        foreach (object key in Instance.graphDictionary.Keys)
            if (key is AttributeKey)
                toRemove.Add(key);
        foreach (object key in toRemove) Instance.InstanceRemoveGraph(key);
        toRemove.Clear();
        foreach (object key in Instance.persistentLogs.Keys)
            if (key is AttributeKey)
                toRemove.Add(key);
        foreach (object key in toRemove) Instance.persistentLogs.Remove(key);
    ...
    private void RegisterAttributes()
    {
        ...
        var uniqueAttributeContainers = new HashSet<MonoBehaviour>();
        ...
        foreach (MonoBehaviour mb in sceneObjects)
        {
            ...
            // Fields
            {
                ...
                var objectFields = monoType.GetFields(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

                // search all fields/properties for the [DebugGUIVar] attribute
                for (int i = 0; i < objectFields.Length; i++)
                {
                    DebugGUIPrintAttribute printAttribute =
                        Attribute.GetCustomAttribute(objectFields[i], typeof(DebugGUIPrintAttribute)) as DebugGUIPrintAttribute;
                    ...
                    DebugGUIGraphAttribute graphAttribute =
                        Attribute.GetCustomAttribute(objectFields[i], typeof(DebugGUIGraphAttribute)) as DebugGUIGraphAttribute;
                    ...
                    if (graphAttribute != null)
                    {
                        ...
                        {
                            Debug.LogError(string.Format("Cannot cast {0}.{1} to float. This member will be ignored.", monoType.Name,
                                objectFields[i].Name));
                            continue;
                        }
                        ...
                        AttributeKey key = new AttributeKey(objectFields[i]);
            ...
            // Properties
            {
                var objectProperties = monoType.GetProperties(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);

                for (int i = 0; i < objectProperties.Length; i++)
                {
                    DebugGUIPrintAttribute printAttribute =
                        Attribute.GetCustomAttribute(objectProperties[i], typeof(DebugGUIPrintAttribute)) as DebugGUIPrintAttribute;
                    ...
                    DebugGUIGraphAttribute graphAttribute =
                        Attribute.GetCustomAttribute(objectProperties[i], typeof(DebugGUIGraphAttribute)) as DebugGUIGraphAttribute;
                    ...
                    if (graphAttribute != null)
                    {
                        ...
                        AttributeKey key = new AttributeKey(objectProperties[i]);
                        if (!attributeKeys.ContainsKey(mb))
                            attributeKeys.Add(mb, new List<AttributeKey>());
                        attributeKeys[mb].Add(key);
        ...
        foreach (MonoBehaviour mb in uniqueAttributeContainers)
        {
    ...
    private void CleanUpDeletedAtributes()
    {
        for (int i = 0; i < attributeContainers.Count; i++)
        {
            if (attributeContainers[i] == null)
            {
                ...
                foreach (AttributeKey key in keys) InstanceRemoveGraph(key);
    ...
    private class GraphContainer
    {
        ...
        private static Color32[] clearColorArray = new Color32[graphWidth * graphHeight];

        public GraphContainer(int width, int height)
        {
            values = new float[width];
            events = new float[width];
        ...
        public void Push(float val)
        {
            ...
            float nextVal = values[Mod(currentIndex - 1, values.Length)];
        ...
        // Add a data point to the beginning of the graph
        public void PushEvent(float val)
        {
            if (autoScale && (val > max || val < min)) SetMinMax(Mathf.Min(val, min), Mathf.Max(val, max));

            currentIndex = (currentIndex + 1) % events.Length;
            events[currentIndex] = val;
            ...
            // Read from index backwards
            float value = events[Mod(currentIndex, events.Length)];
        ...
        private void RegenerateGraph()
        {
            ...
            for (int i = 0; i < events.Length - 1; i++)
            {
                DrawCircle(
                    ...
                    color,
                    0,
                    (int)(Mathf.InverseLerp(min, max, events[Mod(currentIndex - i, events.Length)]) * graphHeight)
                );
            }
        }
        ...
        // From: https://stackoverflow.com/a/56616769/10439539
        public void DrawCircle(Texture2D tex, Color col, int x, int y, int radius = 3)
        {
    ...
    private void OnDestroy()
    {
        ...
        foreach (object key in _instance.graphDictionary.Keys.ToList()) RemoveGraph(key);
    }
}