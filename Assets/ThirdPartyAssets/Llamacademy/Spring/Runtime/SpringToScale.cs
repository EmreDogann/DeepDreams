...
namespace DeepDreams.ThirdPartyAssets.Llamacademy.Spring.Runtime
{
    public class SpringToScale : BaseSpringBehaviour, ISpringTo<Vector3>, INudgeable<Vector3>
    {
        ...
        {
            CheckInspectorChanges();
            if (Mathf.Approximately(Spring.CurrentVelocity.sqrMagnitude, 0)) StartCoroutine(HandleNudge(Amount));
            else Spring.UpdateEndValue(Spring.EndValue, Spring.CurrentVelocity + Amount);
        }
        ...
        private IEnumerator DoSpringToTarget(Vector3 TargetScale)
        {
            ...
            else Spring.UpdateEndValue(TargetScale, Spring.CurrentVelocity);
